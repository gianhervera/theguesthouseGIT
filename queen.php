<?php
    ...
    require('connect.php');
    session_start();
    $query = "SELECT * FROM room WHERE RoomID = 4";
    ...
    if ($image_upload_detected) { 
        ...
        if (file_is_an_image($temporary_image_path, $new_image_path)) {
           ...
           if(move_uploaded_file($temporary_image_path, $new_image_path)){
                ...
                $query1 = "UPDATE room SET images = '".$image_filename."' WHERE RoomID = 4";
        ...
        $username = filter_input(INPUT_POST, 'name', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            ...
            if((strlen($comment) > 0))
                ...
                if ($_POST["captcha_code"] == $_SESSION["captcha_code"]) {
                    $query = "INSERT INTO room (comment, name, RoomID) VALUES (:comment, :name, '4')";
                    $statement = $db -> prepare($query);
...
<?php endif ?>
        ...
        <img src="uploads/<?= $row['images']?>" alt="Queen Room height="400" width ="400"">
        </div>
    <?php endif ?>
    <h3>Reviews:</h3>
    ...
    <?php endwhile ?>
     <form method="post" action="queen.php">   
